...
const CashOutForm = () => {
  ...
  const [foodSalesInput, setFoodSalesInput] = useState<number | ''>('');
  const [totalSalesBeforeTax, setTotalSalesBeforeTax] = useState<number | ''>('');
  ...
  const loadSubmission = async (id: string) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        setFoodSalesInput(data.foodSales ?? '');
        setTotalSalesBeforeTax(data.totalSalesBeforeTax ?? '');
  ...
  // Handle Enter key to move to next field (skip read-only fields)
  const handleEnterKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      // Find the next editable field (skip read-only fields)
  ...
  const foodSales = Number(foodSalesInput) || 0;
  const liquorSales = Math.max(0, tSales - foodSales);

  // Calculate tip rate based on staff name (Natasha Tennant gets 3%, everyone else 4%)
  const normalizedName = name.trim().toLowerCase();
  const tipRate = normalizedName === 'natasha tennant' ? 0.03 : 0.04;
  const kitchenTipOut = foodSales > 250 ? foodSales * tipRate : 0;
  ...
  const creditTotal = useMemo(() => {
    return [debit, visa, mastercard, amex, oVisa, oDebit, oMastercard, oAmex, skip, uber, doordash, giftCards, rndAdjst, cashPayments]
      .reduce((sum: number, val) => sum + (Number(val) || 0), 0);
  }, [debit, visa, mastercard, amex, oVisa, oDebit, oMastercard, oAmex, skip, uber, doordash, giftCards, rndAdjst, cashPayments]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const reportData: any = {
        ...
        totalSales: tSales,
        foodSales,
        totalSalesBeforeTax: Number(totalSalesBeforeTax) || 0,
        liquorSales,
        tipRateUsed: tipRate,
        kitchenTipOut,
        totalTipOut,
      ...
      <div className="bg-white shadow rounded-lg px-4 py-5 sm:p-6">
        ...
        <form onSubmit={handleSubmit} onKeyDown={(e) => {
          ...
          <div className="bg-gray-50 p-6 rounded-lg mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="flex flex-col">
                <label className="text-xs font-bold uppercase tracking-wider mb-1 text-gray-700">Food Sales ($)</label>
                <input
                  type="number"
                  step="0.01"
                  value={foodSalesInput}
                  onChange={(e) => setFoodSalesInput(e.target.value === '' ? '' : Number(e.target.value))}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      const inputs = Array.from(document.querySelectorAll('input:not([disabled]), select'));
              ...
              {/* Total Sales Before Tax Input */}
              <div className="flex flex-col">
                <label className="text-xs font-bold uppercase tracking-wider mb-1 text-gray-700">Total Sales Before Tax ($)</label>
                <input
                  type="number"
                  step="0.01"
                  value={totalSalesBeforeTax}
                  onChange={(e) => setTotalSalesBeforeTax(e.target.value === '' ? '' : Number(e.target.value))}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      const inputs = Array.from(document.querySelectorAll('input:not([disabled]), select'));
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-700">Kitchen Tip Out ({(tipRate * 100).toFixed(0)}%):</span>
              ...
              {/* Payments Total Display */}
              <div className="col-span-full mt-6 pt-4 border-t">
                <div className="flex justify-between items-center">
                  <span className="text-base font-bold text-gray-900">Payments Total:</span>
          ...
          <div className="bg-white p-6 rounded-lg shadow border mb-6">
            ...
            {/* Net Cash Owing Calculation Display */}
            <div className="mt-6 p-4 bg-gray-50 rounded-lg border">
              <h4 className="text-md font-bold text-gray-900 mb-3">Net Cash Owing Calculation</h4>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-700">Cash Payments:</span>
                  <span className="text-sm font-bold text-gray-900">${(Number(cashPayments) || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-700">- Non-Cash Tips:</span>
                  <span className="text-sm font-bold text-red-600">-${(Number(nonCashTips) || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-700">+ Server Tip Charges:</span>
                  <span className="text-sm font-bold text-green-600">+${(Number(serverTipCharges) || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-700">- Auto Gratuity:</span>
                  <span className="text-sm font-bold text-red-600">-${(Number(autoGratuity) || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center border-t pt-2 mt-2">
                  <span className="text-sm font-bold text-gray-900">Server Net Cash Owing:</span>
          ...
          <div className="bg-white p-6 rounded-lg shadow border mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {/* Cash (Read-only from Server Calculations) */}
              <div className="flex flex-col">
                <label className="text-xs font-bold uppercase tracking-wider mb-1 text-gray-700">Cash</label>
                <input
                  type="number"
                  step="0.01"
                  value={Number(cashPayments) || 0}
                  disabled
                  className="block w-full rounded-md shadow-sm sm:text-sm border p-3 bg-gray-100 border-gray-300 text-gray-900"
                  placeholder="0.00"
                />
              </div>
              ...
              {/* Giftcard/ Givex */}
              <div className="flex flex-col">
                <label className="text-xs font-bold uppercase tracking-wider mb-1 text-gray-700">Giftcard/ Givex</label>
                <input
                  type="number"
                  step="0.01"
                  value={giftCards}
              ...
              {/* Soft break for O values */}
              <div className="col-span-full">
                <h4 className="text-sm font-bold text-gray-700 mt-4 mb-2">Other Payments</h4>
              </div>
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-lg border">
              <div className="flex justify-between items-center">
                <span className="text-sm font-bold text-gray-900">Payments Total:</span>
                <span className="text-lg font-bold text-green-600">${creditTotal.toFixed(2)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-red-50 border-l-4 border-red-500 p-6 rounded-lg mb-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className={`text-base font-bold ${serverNetCashOwing > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  ${serverNetCashOwing.toFixed(2)}
                </span>
              </div>
              
              <div className="flex justify-between items-center">
                <span className="text-base font-medium text-gray-900">Total Tip Out:</span>
                <span className="text-base font-bold text-red-600">${totalTipOut.toFixed(2)}</span>
              </div>
              
              <div className="flex justify-between items-center border-t pt-4">
                <span className="text-lg font-bold text-gray-900">Final Amount Owed:</span>
                <span className={`text-2xl font-bold ${finalOwed > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  ${finalOwed.toFixed(2)}
                </span>
              </div>
            </div>
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  ...
                  {editingId ? 'Update Cash Out Sheet' : 'Submit Cash Out Sheet'}
                </>
              )}
            </button>
          </div>
